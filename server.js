...
app.get('/api/ips', requireAdmin, async (req, res) => {
  try {
    ...
    console.error('GET /api/ips error:', e);
...
app.post('/api/ips', requireAdmin, async (req, res) => {
  ...
  try {
    await pool.query(
      `INSERT INTO ip_whitelist (ip, label, added_at) VALUES ($1, $2, $3)
       ON CONFLICT (ip) DO UPDATE SET label = $2, added_at = $3`,
      [ip.trim(), label || '', now()]
    );
    res.json({ ok: true, ip: ip.trim(), label: label || '' });
  } catch (e) {
    console.error('POST /api/ips error:', e);
